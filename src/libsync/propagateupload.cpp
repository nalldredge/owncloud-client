...
void PropagateBundleQNAM::start()
{
...
QString PropagateBundleQNAM::getRemotePath(QString filePath){
    return QString((_propagator->_remoteFolder) + filePath);
}

bool PropagateBundleQNAM::append(SyncFileItemPtr bundledFile){
...
bool PropagateBundleQNAM::empty(){
    return _bundledFiles.empty();
}

void PropagateBundleQNAM::slotMultipartFinished()
{
...
void PropagateBundleQNAM::finalize(const SyncFileItem &copy)
{
    //TODO
    done(SyncFileItem::Success);
}

void PropagateBundleQNAM::slotJobDestroyed(QObject* job)
{
    _jobs.erase(std::remove(_jobs.begin(), _jobs.end(), job) , _jobs.end());
}

void PropagateBundleQNAM::abort()
{
    foreach(auto *job, _jobs) {
        if (job->reply()) {
            qDebug() << Q_FUNC_INFO << job << this->_item->_file;
...
// This function is used whenever there is an error occuring and jobs might be in progress
void PropagateBundleQNAM::abortWithError(SyncFileItem::Status status, const QString &error)
{
    _finished = true;
    abort();
    done(status, error);
